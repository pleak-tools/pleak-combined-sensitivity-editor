...
export class TaskHandler {
          ...
          if (result.parse_tree[0].CreateFunctionStmt) {
            ...
            if (innerResult.parse_tree.length) {
              return {
                success: {
                  id: this.task.id,
                  taskName: this.task.name.trim().replace(/\s+/g, "_"),
                  query: obj_query
                }
              };
            } else {
  ...
  loadTaskOptionsPanelTemplate() {
    if ($('#input-options').has('#task-options-panel').length) {
      ...
      $('#input-options').prepend($('<div>').load(config.frontend.host + '/' + config.combined_sensitivity_editor.folder + '/src/app/editor/templates/task-options-panel.html', () => {
        this.initTaskOptionsPanel();
      }));
    }
  }
